...
function PaymentProcessingContent() {
  ...
  const [phase, setPhase] = useState<"loading" | "testButtons">("loading");

  useEffect(() => {
    // Simulate payment gateway processing for 2.5 seconds
    const timer = setTimeout(() => {
      setPhase("testButtons");
    }, 2500);
  ...
  return (
    <div className="min-h-screen bg-obsidian-950 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-obsidian-900 border border-gold/30 rounded-lg p-8 text-center space-y-8">
        {phase === "loading" ? (
          <>
            ...
            <div className="space-y-3">
              ...
              <p className="text-silver-dark text-sm">
                Please wait while we verify your payment through the secure
                gateway...
              </p>
            </div>
            <div className="flex justify-center gap-1 pt-2">
              <span className="text-[10px] font-mono text-silver-dark">●</span>
              <span className="text-[10px] font-mono text-silver-dark animate-pulse">●</span>
              <span className="text-[10px] font-mono text-silver-dark">●</span>
            </div>
          </>
        ) : (
